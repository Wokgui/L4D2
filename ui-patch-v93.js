...
/* Le bouton Tirage de la barre du bas sert aussi de bouton Accueil.
   On restaure directement l'écran initial au lieu de recharger la page :
   cela évite les rares mauvais calculs de hauteur du navigateur Android. */
(()=>{
  const tirage=document.querySelector('.nav button[data-p="d"]');
  const res=document.getElementById('res');
  const drawPage=document.getElementById('d');
  if(!tirage||!res)return;

  let homeHTML='';
  const rememberHome=()=>{
    if(res.classList.contains('home-res')&&!res.querySelector('.result-card'))homeHTML=res.innerHTML;
  };
  rememberHome();
  requestAnimationFrame(rememberHome);
  setTimeout(rememberHome,120);
  setTimeout(rememberHome,500);

  const restoreHome=()=>{
    if(homeHTML){
      res.classList.add('home-res');
      res.innerHTML=homeHTML;
    }
    if(drawPage)drawPage.scrollTop=0;
    res.scrollTop=0;
  ...
  tirage.addEventListener('click',()=>{
    /* Le gestionnaire normal de navigation affiche d'abord l'onglet Tirage,
       puis on remet son contenu d'accueil dans le même rendu. */
